...
@section('content')
<style>
    .badge-forma { padding: 4px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: 600; text-transform: capitalize; display: inline-flex; align-items: center; gap: 4px; }
    ...
    .summary-row { display: flex; justify-content: space-between; align-items: center; padding: 14px 20px; border-bottom: 1px solid var(--border-light); }
    .summary-row:last-child { border-bottom: none; }
    .summary-row.highlight { background: linear-gradient(135deg, #faf5ff, #f3e8ff); }
    .summary-row .slabel { display: flex; align-items: center; gap: 10px; font-size: 0.9rem; color: var(--text); }
    .summary-row .svalue { font-weight: 700; font-size: 0.95rem; }
    .section-title { font-size: 1rem; font-weight: 700; color: var(--primary); margin-bottom: 0; padding-bottom: 12px; border-bottom: 2px solid rgba(var(--primary-rgb), 0.08); display: flex; align-items: center; gap: 10px; }
...
<div class="stats-bar">
    ...
    <div class="stat-card" style="background: var(--primary); border-color: var(--primary);">
        <div class="stat-icon" style="background: rgba(255,255,255,0.2); color: white;"><i class="fas fa-bullseye"></i></div>
        <div class="stat-value" style="color: white;">R$ {{ number_format($resumo['ticketMedio'], 2, ',', '.') }}</div>
...
<div class="table-container">
    ...
    <div class="summary-row">
        <span class="slabel"><i class="fas fa-list-check" style="color: var(--primary);"></i> Total de Cobranças</span>
        <span class="svalue" style="font-weight: 700;">{{ $pagamentosPeriodo['total_pagamentos'] }}</span>
    </div>
    <div class="summary-row">
        <span class="slabel"><i class="fas fa-circle-check" style="color: var(--success);"></i> Total Pago</span>
        <span class="svalue" style="color: var(--success);">R$ {{ number_format($pagamentosPeriodo['total_pago'], 2, ',', '.') }}</span>
    </div>
    <div class="summary-row">
        <span class="slabel"><i class="fas fa-clock" style="color: var(--warning);"></i> Total Pendente</span>
        <span class="svalue" style="color: var(--warning);">R$ {{ number_format($pagamentosPeriodo['total_pendente'], 2, ',', '.') }}</span>
    </div>
    <div class="summary-row">
        <span class="slabel"><i class="fas fa-circle-xmark" style="color: var(--danger);"></i> Total Vencido</span>
        <span class="svalue" style="color: var(--danger);">R$ {{ number_format($pagamentosPeriodo['total_vencido'], 2, ',', '.') }}</span>
    </div>
    <div class="summary-row highlight">
        <span class="slabel"><i class="fas fa-coins" style="color: var(--primary);"></i> <strong>Valor Total Recebido</strong></span>
        <span class="svalue" style="font-size: 1.2rem; color: var(--primary);">R$ {{ number_format($pagamentosPeriodo['valor_recebido'], 2, ',', '.') }}</span>
    </div>
</div>
...
<div class="table-container">
    ...
    <div class="summary-row">
        <span class="slabel"><i class="fas fa-circle-check" style="color: var(--success);"></i> Clientes Renovados / Pagos</span>
        <span class="svalue" style="color: var(--success);">{{ $churnRenovacoes['renovados'] }}</span>
    </div>
    <div class="summary-row">
        <span class="slabel"><i class="fas fa-arrow-trend-down" style="color: var(--danger);"></i> Churn (Pós-pagamento)</span>
        <span class="svalue" style="color: var(--danger);">{{ $churnRenovacoes['churn'] }}</span>
    </div>
    <div class="summary-row">
        <span class="slabel"><i class="fas fa-ban" style="color: var(--text-secondary);"></i> Desistência (Pré-pagamento)</span>
        <span class="svalue" style="color: var(--text-secondary);">{{ $churnRenovacoes['desistencias'] }}</span>
    </div>
    <div class="summary-row">
        <span class="slabel"><i class="fas fa-percent" style="color: var(--primary);"></i> Taxa de Churn</span>
        <span class="svalue" style="color: {{ $churnRenovacoes['churn_percentual'] > 20 ? 'var(--danger)' : ($churnRenovacoes['churn_percentual'] > 10 ? 'var(--warning)' : 'var(--success)') }};">{{ $churnRenovacoes['churn_percentual'] }}%</span>
    </div>
    <div class="summary-row">
        <span class="slabel"><i class="fas fa-circle" style="color: var(--success);"></i> Recorrência Ativa</span>
        <span class="svalue" style="font-weight: 700;">{{ $churnRenovacoes['ativos'] }}</span>
    </div>
    <div class="summary-row">
        <span class="slabel"><i class="fas fa-circle" style="color: var(--danger);"></i> Recorrência Inativa</span>
        <span class="svalue" style="font-weight: 700;">{{ $churnRenovacoes['inativos'] }}</span>
    </div>
</div>
