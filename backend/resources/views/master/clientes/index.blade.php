@extends('layouts.app')
@section('title', 'Clientes da Operação')
...
<x-page-hero 
    title="Clientes da Operação" 
    subtitle="Carteira completa de clientes" 
...
<!-- Filters -->
<form method="GET" action="{{ route('master.clientes') }}">
<div class="filters-bar">
    ...
    <a href="{{ route('master.clientes') }}" class="btn btn-ghost btn-sm">Limpar</a>
</div>
</form>
...
<div class="table-container">
    ...
    <table>
        <thead>
            <tr>
                ...
                <th><i class="fas fa-user-tie" style="margin-right: 4px;"></i> Vendedor</th>
        ...
        <tbody>
            ...
            <tr>
                <td>
                    ...
                    <div style="font-size: 0.8rem; color: var(--text-muted);"><i class="fas fa-location-dot"></i> {{ $c->localidade ?? '—' }}</div>
                </td>
                <td>
                    <div style="font-weight: 600;">{{ $c->nome_pastor ?? '—' }}</div>
                    <div style="font-size: 0.8rem; color: var(--text-muted);"><i class="fas fa-phone"></i> {{ $c->whatsapp ?? '' }}</div>
                </td>
                <td style="font-size: 0.85rem; color: var(--text-muted);">{{ $c->vendas->first()?->vendedor?->user?->name ?? '—' }}</td>
                <td>
                    @if($c->temCobrancaAberta())
                        <span class="badge" style="background: #fee2e2; color: #b91c1c;"><i class="fas fa-triangle-exclamation"></i> Débito</span>
                    @else
                        <span class="badge" style="background: #dcfce7; color: #15803d;"><i class="fas fa-circle-check"></i> Em Dia</span>
                    @endif
                </td>
                <td><span class="badge badge-{{ $c->status ?? 'ativo' }}">{{ ucfirst($c->status ?? 'Ativo') }}</span></td>
                <td style="text-align: right;">
                    <a href="{{ route('master.clientes.show', $c->id) }}" class="action-btn"><i class="fas fa-eye"></i> Detalhes</a>
                </td>
            </tr>
            @endforeach
        </tbody>
    ...
    <div style="padding: 16px 20px; border-top: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center;">
        <span style="font-size: 0.85rem; color: var(--text-muted);">Mostrando {{ $clientes->firstItem() ?? 0 }} a {{ $clientes->lastItem() ?? 0 }} de {{ $clientes->total() }}</span>
    ...
    <div class="empty-state">
        ...
        <p>Os clientes aparecerão aqui quando forem cadastrados.</p>
    </div>
    @endif
</div>
