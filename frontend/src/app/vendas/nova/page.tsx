...
export default function GestorNovaVendaPage() {
  ...
  const handleSalvar = async () => {
    try {
      await VendasService.criar({
        cliente_id: 1, // hardcoded for now just to test
        valor: 1500, // hardcoded mock value
        plano: planoSelecionado || 'Básico',
        forma_pagamento: formaPagamento || 'Boleto',
        observacao: "Venda gerada manualmente"
      });
      toast.success(t("Venda cadastrada com sucesso!"));
      router.push("/gestao-comercial/vendas");
    } catch (e) {
  ...
  useEffect(() => {
    ...
    else setPlanoSelecionado("Enterprise");
  }, [membros]);
  ...
  const getValorTotal = () => {
    ...
    if (planoSelecionado === "Enterprise") return 0;
