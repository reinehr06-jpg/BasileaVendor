...
import { 
  Building2, Plus, Search, MoreVertical, Wallet
} from "lucide-react";
import { CentrosDeCustoService } from "@/services/centros.service";
...
export default function CentrosDeCustoPage() {
  ...
  const carregarCentros = async () => {
    try {
      setLoading(true);
      const res = await CentrosDeCustoService.listar();
      ...
      <div className="flex-1 ml-[240px] flex flex-col h-screen overflow-hidden">
        ...
        <main className="p-4 flex-1 flex flex-col w-full max-w-[1400px] mx-auto gap-4 overflow-hidden relative">
          ...
          <div className="flex items-center justify-between shrink-0 bg-white rounded-[12px] p-4 border border-[#E5E7EB] shadow-sm">
            ...
            <div className="flex items-center gap-3">
              <Link href="/contabilidade/centros-de-custo/nova" className="bg-[#6D28D9] hover:bg-[#5B21B6] transition-colors text-white px-4 py-2 rounded-[8px] text-[13px] font-[700] flex items-center gap-2 shadow-sm">
          ...
          <div className="flex items-center gap-3 shrink-0 bg-white rounded-[12px] border border-[#E5E7EB] shadow-[0_2px_10px_rgba(0,0,0,0.02)] p-3 mb-4">
            <div className="flex items-center gap-2 px-2 border-r border-[#E5E7EB] shrink-0">
              <div className="text-[#6D28D9]"><Wallet className="w-[16px] h-[16px]" /></div>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 bg-[#F9FAFB] shadow-[0_1px_0_#F1F1F4] z-10">
                  <tr>
                    ...
                    <th className="py-3 px-5 text-[11px] font-[700] text-[#6B7280] uppercase tracking-wider border-b border-[#F1F1F4] text-right">Orçamento Mensal</th>
                    ...
                    <tr key={item.id} className="hover:bg-[#F9FAFB] transition-colors group border-b border-[#F1F1F4]">
                      <td className="py-4 px-5">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[11px] font-[500] text-[#6B7280]">Cód: {item.codigo}</span>
                        </div>
                      </td>
                      <td className="py-4 px-5">
                      ...
                      <td className="py-4 px-5 text-right">
                        <span className="text-[13px] font-[600] text-[#111827]">R$ {Number(item.orcamento).toLocaleString('pt-BR', {minimumFractionDigits: 2})}</span>
                      </td>
                      <td className="py-4 px-5 text-center">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-[700] ${item.status === 'Ativo' ? 'bg-[#ECFDF5] text-[#10B981]' : item.status === 'Em Progresso' ? 'bg-[#EFF6FF] text-[#3B82F6]' : 'bg-[#F3F4F6] text-[#6B7280]'}`}>
                          {item.status}
                        </span>
                      </td>
                      <td className="py-4 px-5 text-right">
