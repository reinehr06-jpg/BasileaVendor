...
export default function VendedorDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const resumo = metricas?.data?.resumo;
      setMetrics({
        faturamento: formatCurrency(resumo?.receitaTotal || 0),
        vendas: resumo?.totalVendas || 0,
        clientes: dashboard?.kpis?.total_clientes ?? 0,
        ticketMedio: formatCurrency(resumo?.ticketMedio || 0),
        churn: `${(resumo?.churn || 0).toFixed(1)}%`,
        renovacoes: metricas?.data?.renovacoes || 0,
        chartData: metricas?.data?.receitaMensal?.map((m: any) => ({ name: m.name, valor: m.total })) || [],
      });
    });
  }, []);
            ...
            <div className="w-[300px] flex flex-col gap-[16px] shrink-0">
              ...
              <div className="bg-white rounded-[14px] border border-[#E5E7EB] p-[20px] shadow-sm flex items-center gap-[16px] hover:shadow-md transition-shadow">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[22px] font-[800] text-[#1A1A2E] leading-tight">{metrics.ticketMedio}</span>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[14px] border border-[#E5E7EB] p-[20px] shadow-sm flex items-center gap-[16px] hover:shadow-md transition-shadow">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[22px] font-[800] text-[#1A1A2E] leading-tight">{metrics.renovacoes} <span className="text-[14px] font-[700] text-[#6B7280]">UND</span></span>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[14px] border border-[#E5E7EB] p-[20px] shadow-sm flex items-center gap-[16px] hover:shadow-md transition-shadow">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[22px] font-[800] text-[#1A1A2E] leading-tight">{metrics.churn}</span>
                </div>
              </div>
